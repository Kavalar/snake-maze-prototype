...
function canStartInDirection(dirName) {
  const d = DIRS[dirName];
  if (!d || !snake.length) return false;
  const head = snake[0];
  const nx = head.x + d.x;
  const ny = head.y + d.y;
...
function handleDirectionInput(dirName) {
  ...
  if (!isPlaying) {
    if (!canStartInDirection(dirName)) {
      statusEl.textContent = 'У цей бік старт неможливий. Обери іншу стрілку.';
      draw();
      return;
    }

    direction = dirName;
    queuedDirection = dirName;
    tryStartLevel();
    return;
  }
...
function resetLevel(options = {}) {
  ...
  statusEl.textContent = food
    ? `Натисни стрілку для старту. Ціль: зібрати 0/${detailsGoal} деталей і дійти до виходу.`
